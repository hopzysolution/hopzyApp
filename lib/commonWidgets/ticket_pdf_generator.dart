  ...
  Future<void> _generateAndSavePdf(
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        margin: const pw.EdgeInsets.all(10),
        build: (context) => [
          _buildHeader(ticket, logoImage),
          pw.SizedBox(height: 15),
          _buildPnrWidget(ticket),
          pw.SizedBox(height: 20),
          _buildTripInformation(ticket),
          pw.SizedBox(height: 20),
          _buildPassengerDetails(ticket),
          pw.SizedBox(height: 20),
          _buildPaymentInformation(ticket),
          pw.SizedBox(height: 20),
...
// === PDF Layout Widgets (structure unchanged) ===
...
pw.Widget _buildHeader(Map<String, dynamic> data, pw.ImageProvider? logoImage) {
  return pw.Container(
    ...
    child: pw.Row(
      ...
      children: [
        if (logoImage != null)
          pw.Container(width: 150, height: 70, child: pw.Image(logoImage, fit: pw.BoxFit.contain))
        else
          pw.Container(
            width: 80,
            height: 60,
        ...
        pw.Column(
          crossAxisAlignment: pw.CrossAxisAlignment.start,
          children: [
            pw.Text("HOPZY Solutions Pvt Ltd", style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 5),
            pw.Text("Operator Helpline: +91-XXXXXXXXXX", style: pw.TextStyle(fontSize: 12)),
            pw.Text("Email: support@hopzy.com", style: pw.TextStyle(fontSize: 12)),
            pw.Text("Support: +91-YYYYYYYYYY", style: pw.TextStyle(fontSize: 12)),
          ],
        ),
      ],
    ),
...
pw.Widget _buildTripInformation(Map<String, dynamic> data) {
  ...
  return pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
      pw.Text('Trip Information', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 8),
      _buildTableN([
        ['Operator Name', data['operatorName'] ?? ''],
        ['Ticket ID', data['ticketId'] ?? ''],
      ...
      pw.SizedBox(height: 16),
      pw.Text('Boarding Details', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 8),
      _buildTableN([
        ['Boarding Point', boarding['venue'] ?? boarding['name'] ?? ''],
        ['Boarding Address', boarding['address'] ?? ''],
        ['Boarding Time', boarding['time'] ?? ''],
      ]),
      pw.SizedBox(height: 16),
      pw.Text('Dropping Details', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 8),
      _buildTableN([
        ['Dropping Point', dropping['venue'] ?? dropping['name'] ?? ''],
        ['Dropping Address', dropping['address'] ?? ''],
        ['Dropping Time', dropping['time'] ?? ''],
      ]),
    ],
  );
}
...
pw.Widget _buildPassengerDetails(Map<String, dynamic> data) {
  final passengers = List<Map<String, dynamic>>.from(data['passengers'] ?? []);

  List<pw.TableRow> rows = [
    pw.TableRow(
      decoration: const pw.BoxDecoration(color: PdfColors.lightBlue100),
      children: ['#', 'Name', 'Gender', 'Age', 'Seat', 'Fare', 'Status']
          .map((h) => pw.Padding(
        padding: const pw.EdgeInsets.all(6),
        child: pw.Text(h, style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
      ))
          .toList(),
    ),
  ];
  ...
  for (int i = 0; i < passengers.length; i++) {
    ...
    rows.add(
      pw.TableRow(
        children: [
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text((i + 1).toString())),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['Name'] ?? '')),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['gender'] ?? '')),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['age']?.toString() ?? '')),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['seatNo'] ?? '')),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text("₹ ${p['fare'] ?? '0'}")),
          pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['status'] ?? '')),
        ],
      ),
    );
  }
  ...
  return pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
      pw.Text('Passenger Details', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 8),
      pw.Table(border: pw.TableBorder.all(width: 1), children: rows),
    ],
  );
}
...
pw.Widget _buildCancellationPolicy(Map<String, dynamic> data) {
  // final policies = List<Map<String, dynamic>>.from(data['cancellationpolicy'] ?? []);

  List<pw.TableRow> rows = [
    pw.TableRow(
      decoration: const pw.BoxDecoration(color: PdfColors.lightBlue100),
      children: ['#', 'Description', 'Refund', 'Charge']
          .map((h) => pw.Padding(
        padding: const pw.EdgeInsets.all(6),
        child: pw.Text(h, style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold)),
      ))
  ...
  //       pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text((i + 1).toString())),
  //       pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['description'] ?? '')),
  //       pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['refundpercent'] ?? '')),
  //       pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(p['cancellationcharge'] ?? '')),
  //     ]),
  //   );
  // }
  ...
  return pw.Column(
    crossAxisAlignment: pw.CrossAxisAlignment.start,
    children: [
      pw.Text('Cancellation Policy', style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
      pw.SizedBox(height: 8),
      pw.Table(border: pw.TableBorder.all(width: 1), children: rows),
    ],
  );
...
pw.Widget _buildPnrWidget(Map<String, dynamic> data) {
  ...
  return pw.Container(
    ...
    decoration: const pw.BoxDecoration(color: PdfColors.blue200),
    child: pw.Text(
      "PNR: ${data['pnr'] ?? ''} | Departure: ${boarding['time'] ?? ''}",
      style: const pw.TextStyle(fontSize: 10, color: PdfColors.white),
      textAlign: pw.TextAlign.center,
    ),
  );
